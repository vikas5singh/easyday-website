...
export default function OptionsModal({ show, onAddToCart, cartItem }) {
  const [open, setOpen] = show;
  const { item, quantity } = open || {};
  ...
  const sortedGroupAddons = useMemo(() => {
    ...
    (item?.variations || [])?.forEach((item) => {
      if (!groupedAddons[item?.attributes?.[0]?.name]) {
        groupedAddons[item?.attributes?.[0]?.name] = [];
      }
      groupedAddons[item.attributes?.[0]?.name]?.push(item);
    });

    return groupedAddons;
  }, [item?.variations]);

  console.log("Addons List1", item);
  console.log("Addons List data", data);

  console.log("Addons List data12", item?.variations);
  ...
  console.log("Priceee", totalPrice);
      ...
      <Modal.Body className="x-options-modal-body">
        <div className="d-flex align-items-start justify-content-between x-options-modal-body-content">
          <div className="d-flex flex-column">
            <h4 className="mb-0">{item.itemName}</h4>
            <h5>${item.price}</h5>
          </div>
          <Button
                      ...
                      <Col xs={12} sm={6} key={index}>
                        <FormCheck>
                          <FormCheck.Input
                            ...
                            id={`customise-${custom?._id}-${option?._id}`}
                          ...
                          />
                          <FormCheck.Label
                            htmlFor={`customise-${custom?._id}-${option?._id}`}
                            className="w-100"
                          >
              ...
              <div className="shadow-sm rounded x-item-option-card p-2 mt-3">
                ...
                {(item?.variations || [])?.map((group, index) => (
                  <Row className="py-1 mt-1" key={index}>
                    {/* <Col xs="12">
                      <small className="text-muted">{group?.attributes?.[0]?.name}</small>
                    </Col> */}
