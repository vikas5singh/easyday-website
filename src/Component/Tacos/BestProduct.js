...
export default function BestSellert(props) {
        ...
        responsive: [
            {
                ...
                settings: {
                    slidesToShow: 3,
                    slidesToScroll: 1,
                    rows: 1, // Add this to responsive too
                },
            },
            {
                breakpoint: 768,
    ...
    const ProductCard = ({ item, index }) => (
        <div className="tacos-card" key={index} style={{
            ...
            <h5 className="m-0 py-1 mt-5 fw-bold">
                {item.promotionName}
            </h5>
            {/* <div className="quantity-control">
                    <h5 className="product-price m-0 fw-sbold">
                        ${item.price.toFixed(2)}{" /"}{item.compare_price && (
                            <span className="compare-price ms-2">
                                <del>${item.compare_price.toFixed(2)}</del>
                            </span>
                        )}</h5>
                </div> */}
            {/* </div> */}
    ...
    return (
        <section className="best-sellert-section gifsection tabbing-tacos">
            <div className="section-header text-center heading-wrap">
                <h2>Our Best Products</h2>
                <p>Discover Best products loved by our customers</p>
                <p>&nbsp;</p>
            </div>
