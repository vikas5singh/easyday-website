...
import { editAddress, getAddress, viewAddress } from "../../../Redux/actions";
...
export default function EditAddressModal(props) {
  ...
  const addrDetail = useSelector((s) => s.address.editAddr);
  ...
  useEffect(() => {
    setAddr((pre) => ({
      ...
      addressLocation: {
        lng: addrDetail?.addressLocation?.coordinates[0],
        lat: addrDetail?.addressLocation?.coordinates[1],
      },
      area: addrDetail?.area,
      houseNo: addrDetail?.houseNo,
  ...
  const handleSubmit = (e, id) => {
    ...
    dispatch(editAddress({ ...addr, addressId: id }, callBack));
  };

  useEffect(() => {
    dispatch(viewAddress(detail?._id));
  }, [detail]);
  return (
    <Modal
      {...props}
            ...
            <Row>
              <Col lg={12} md={12} sm={12}>
                <div className="right-address">
                  ...
                  <Button
                    className="btn-done"
                    onClick={(e) => handleSubmit(e, addrDetail._id)}
                  >
                    Save Address
                  </Button>
                </div>
