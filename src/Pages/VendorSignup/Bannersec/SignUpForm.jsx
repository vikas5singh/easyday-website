...
const SignUpForm = ({ showSignUp, setShowSignUp }) => {
  ...
  const homeData = useSelector((s) => s.restaurant.homeData?.resdata);
  let storeList = (homeData || [])?.filter((s) => !["SERVICEPROVIDER", "TAXI", "PIZZA", "FOOD", "PICKUPDROP"].includes(s.storeType)).map(({ _id, label, storeType }) => ({
    label: label,
    value: _id,
  }))
  const initialValues = {
          ...
          <Fragment>
            ...
            >
              <Modal.Header className="border-0" closeButton>
                <Modal.Title>
                  Sign <span className="theme-clr"> Up </span> To <span className="theme-clr">Vendor</span>
                </Modal.Title>
              </Modal.Header>
              <p className="px-3">Become a vendor and grow your business with us. Fill out the form below to get started!</p>
              <Modal.Body>
                    ...
                    <Col md="12" className="my-2">
                      ...
                      <div className="icon-with-text position-relative">
                        <Select
                          options={storeList}
                          styles={customStyles}
                          onChange={(option) => setFieldValue('storeTypeId', option.value)}
                          value={storeList.find((option) => option.value === values.storeTypeId)}
                        />
                      </div>
